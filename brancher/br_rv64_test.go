...
func TestBrancher_RV64(t *testing.T) {
	tests := []struct {
		name       string
		...
		{
			name: "beq taken 64-bit",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
				Funct3: branch.Beq.F3,
				RS1D:   0x123456789ABCDEF0,
				RS2D:   0x123456789ABCDEF0,
		...
		{
			name: "beq not taken 64-bit",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
				Funct3: branch.Beq.F3,
				RS1D:   0x123456789ABCDEF0,
				RS2D:   0x123456789ABCDEF1,
				Imm:    0b000000001010,
			},
			wantTaken:  false,
			wantOffset: 4,
		...
		{
			name: "blt taken signed 64-bit neg-pos",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
				Funct3: branch.Blt.F3,
				RS1D:   0xFFFFFFFFFFFFFFF0, // -16
				RS2D:   5,
				Imm:    0b000000001010,
			},
			wantTaken:  true,
		...
		{
			name: "bltu not taken unsigned 64-bit neg-pos",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
				Funct3: branch.Bltu.F3,
				RS1D:   0xFFFFFFFFFFFFFFF0, // Large unsigned
				RS2D:   5,
				Imm:    0b000000001010,
			},
			wantTaken:  false,
		...
		{
			name: "bge taken signed 64-bit pos-neg",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
				Funct3: branch.Bge.F3,
				RS1D:   5,
				RS2D:   0xFFFFFFFFFFFFFFF0, // -16
				Imm:    0b000000001010,
			},
			wantTaken:  true,
			wantOffset: 20,
		...
		{
			name: "bgeu taken unsigned 64-bit neg-pos",
			op: datapath.BranchOp{
				PC:     0x1000,
				Opcode: 0x63,
...
func FuzzBrancher(f *testing.F) {
	f.Add(uint64(10), uint64(10), uint64(0x1000), uint32(0b000000001010), uint8(branch.Beq.F3))
	f.Add(uint64(10), uint64(20), uint64(0x1000), uint32(0b000000001010), uint8(branch.Bne.F3))
	f.Fuzz(func(t *testing.T, rs1d, rs2d, pc uint64, imm uint32, f3 uint8) {
		f3 = f3 % 8
