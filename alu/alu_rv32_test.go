...
func TestALUOperations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			result: datapath.AluResult{F3:integer.Add.F3, Rd: 0b00101, Value: 2},
		},
		{
			// add,addi: -3 + -7
			...
			result: datapath.AluResult{F3: integer.Addi.F3, Rd: 0b00101, Value: 0b11111111111111111111111111110110},
		},
		{
			name:   "sub: pos-pos",
			param:  datapath.AluParam{Opcode: 0b0110011, Rd: 0b00101, Funct3: integer.Sub.F3, Funct7: integer.Sub.F7, Op1: 7, Op2: 3},
			result: datapath.AluResult{F3:integer.Sub.F3, Rd: 0b00101, Value: 4},
		},

		{
			...
			result: datapath.AluResult{F3: integer.Sub.F3, Rd: 0b00101, Value: 10},
		},
		{
			// sub: -7 - (-3)
			name:   "sub: neg-neg",
			...
			go func() {
				...
				for result := range resultCh {
					...
					if result.F3 != test.result.F3 {
						t.Errorf("unexpected ALU result Funct3: %d", result.F3)
					}
				}
			}()
...
func TestALU_Run_Multiple(t *testing.T) {
	...
	// load params
	go func(){
		paramsCh <- datapath.AluParam{Opcode: 0b0110011, Rd: 0b00101, Funct3: 0, Funct7: 0, Op1: 7, Op2: 12}
		paramsCh <- datapath.AluParam{Opcode: 0b0110011, Rd: 0b00101, Funct3: integer.Sll.F3, Funct7: integer.Sll.F7, Op1: 2, Op2: 2}
		paramsCh <- datapath.AluParam{Opcode: 0b0110011, Rd: 0b00101, Funct3: integer.Xor.F3, Funct7: integer.Xor.F7, Op1: 0xFF, Op2: 0b00101}
		close(paramsCh)
	}()
