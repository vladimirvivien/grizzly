...
func TestCtrl_ALU(t *testing.T) {
	...
	go func() {
		...
		op1 <- 4; rd2 <- 12 // simulate alu-operand-1; reg data out 2
	...
	rs1, rs2, werf, rd, aluOut :=
		ctrl.GetPin(Out.RS1),
		ctrl.GetPin(Out.RS2),
		ctrl.GetPin(Out.Werf),
		ctrl.GetPin(Out.RD),
		arlou.GetPin(alu.Out.Result)

	// process inst 1, add
	datapath.Collect(rs1, rs2, werf, rd)
	if result := datapath.Collect(aluOut)[0]; result != 16 {
		t.Fatal("unexpected alu result:", result)
	}

	// process inst 2, addi
	datapath.Collect(rs1, rs2, werf, rd)
	if result := datapath.Collect(aluOut)[0]; result != 18 {
		t.Fatal("unexpected alu result:", result)
	}

	// process inst 3, add
	datapath.Collect(rs1, rs2, werf, rd)
	if result := datapath.Collect(aluOut)[0]; result != 28 {
		t.Fatal("unexpected alu result:", result)
	}

	// process inst 4, add
	datapath.Collect(rs1, rs2, werf, rd)
	if result := datapath.Collect(aluOut)[0]; result != 20 {
		t.Fatal("unexpected alu result:", result)
	}
}
